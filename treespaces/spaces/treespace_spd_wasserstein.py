...
class TreeSpdWasserstein(TreeSpaceSpd):
    ...
    def a_dist(cls, p, q, squared=False):
        ...
        sq_p = SpdAf1.ssqrtm(p)
        sq_distance = np.trace(p) + np.trace(q) - 2 * np.trace(SpdAf1.ssqrtm(np.dot(sq_p, q).dot(sq_p)))
    ...
    def a_path_t(cls, p, q, t):
        ...
        p, q = cls.s_lift(p), cls.s_lift(q)
        sqrt_pq = SpdAf1.ssqrtm(p.dot(q))
        return (1 - t) ** 2 * p + t ** 2 * q + t * (1 - t) * (sqrt_pq + sqrt_pq.T)
    ...
    def a_path(cls, p, q, **kwargs):
        ...
        sqrt_pq = SpdAf1.ssqrtm(p.dot(q))
        return [(1 - t) ** 2 * p + t ** 2 * q + t * (1 - t) * (sqrt_pq + sqrt_pq.T) for t in times]

    @classmethod
    def a_inner(cls, u, v, p):
        """ Computes the ambient riemannian metric of vectors u and v at point p. """
        eig_p, r_p = la.eigh(p)  # these are the alphas
        u, v = r_p.T.dot(u).dot(r_p), r_p.T.dot(v).dot(r_p)
        dummy = 1 / (eig_p[:, None] + eig_p[None, :]) ** 2
        return np.sum((u * v * dummy) * eig_p)
    ...
    def a_exp(cls, v, p):
        ...
        sq_inv_p = la.inv(sqrt_p)
        h = la.solve_continuous_lyapunov(a=p, q=np.dot(sqrt_p, v + 2 * p).dot(sqrt_p))
        sq_inv_p_h = np.dot(sq_inv_p, h)
        return np.dot(sq_inv_p_h, sq_inv_p_h.T)
