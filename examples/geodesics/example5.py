...
# x1_list = [np.array([_x * t if _i in [5] else _x for _i, _x in enumerate(x1)])
#            for t in np.linspace(start=0.01, stop=1, endpoint=True, num=n_points // 3)[::-1]]
#
# x2_list = [np.array([_x * t if _i in [5] else _x for _i, _x in enumerate(x2)])
#            for t in np.linspace(start=0.01, stop=1, endpoint=True, num=n_points // 3)]
#
# x3_list = [np.array(
#     [np.cos(np.pi / 2 * t) * _x if _i in [2] else np.sin(np.pi / 2 * t) * _x if _i in [5] else _x for _i, _x in
#      enumerate(x3)])
#            for t in np.linspace(start=0.01, stop=1, endpoint=False, num=(n_points - 3) // 3)[::-1]]
#
# third1 = tuple([Wald(n=n, st=st1, x=_x) for _x in x1_list])
# third2 = tuple([Wald(n=n, st=st2, x=_x) for _x in x2_list])
# thirdm = tuple([Wald(n=n, st=st3, x=_x) for _x in x3_list])
# middle1 = Wald(n=n, st=st3, x=np.array([0 if _i in [2] else _x for _i, _x in enumerate(x3)]))
# middle2 = Wald(n=n, st=st3, x=np.array([0 if _i in [5] else _x for _i, _x in enumerate(x3)]))
...
# cone_path = third1 + (middle1,) + thirdm + (middle2,) + third2
# print(cone_path)
...
if calculate:
    proj_args = {'gtol': 10 ** -12, 'ftol': 10 ** -15, 'btol': 10 ** -6, 'method': 'global-search'}
    for i in range(iterations):
        paths.append(tuple(
            ws.g.s_path(p=wald1, q=wald2, alg='global-straightening', n_points=n_points, start_path=paths[-1],
