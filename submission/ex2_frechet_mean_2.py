...
if calculate_2:
    ...
    interior = np.linspace(0, 10**-3, 100)
    # walds = [Wald(n=n, st=st00, x=[opt_pen]*4)] + [Wald(n=n, st=st01, x=[opt_pen]*4 + [_x]) for _x in interior]
    walds = [Wald(n=n, st=st01, x=[opt_pen]*4 + [_x]) for _x in interior]
    # interior = np.hstack((0, interior))  # insert 0 length interior edge s.t. len(interior) = len(walds)
    frechet_values_2 = [compute_frechet_functional(w=_w) for _w in walds]
    end = time.time()
    t = np.round(end - start, 2)
    print(f"Store the data that has been calculated.")
    pickle.dump(obj=(tuple(interior), tuple(frechet_values_2), t), file=open(fn_data, "wb"))
else:
    print(f"Load the already stored data since the flag 'calculate' is False.")
    interior, frechet_values_2, t = pickle.load(file=open(fn_data, "rb"))
print(f"Calculation took {np.round(t / 60, 2)} minutes.")

fig = plt.figure()
ax = fig.add_subplot(2, 1, 1)
ax.plot(interior, frechet_values_2, color='blue', lw=1.2)
