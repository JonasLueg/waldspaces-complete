...
# ----- THIS FLAG DETERMINES IF THE GEODESICS ARE COMPUTED AGAIN OR LOADED FROM DATA
# FILES INSTEAD -----
calculate = False
...
# ----- COMPUTATION OF THE PATHS FROM THE CONSTRUCTED WALDS TO THE BOUNDARY
# (STAR STRATUM) ----- #
n_iter = 4
j_iter = 4
# these are the parameters used for every projection from ambient space onto the grove
proj_args = {'gtol': 10 ** -9, 'ftol': 10 ** -10, 'btol': 10 ** -7,
             'method': 'global-descent'}
# the parameters for each method are set here:
params = {'n_iter': n_iter, 'j_iter': j_iter, 'proj_args': proj_args}

if calculate:
    start = time.time()
    path_ = tuple(
        ws.g.s_path(p=walds[0], q=walds[1], alg='extend-straighten', **params))
    end = time.time()
    t = np.round(end - start, 2)
    print(f"Calculation took {t} seconds.")
    print(f"Store the data that has been calculated.")
    pickle.dump(obj=(tuple(path_), t), file=open(os.path.join(f"{folder_name}", "data",
                                                              f"path_{n_iter}_iter.p"),
                                                 "wb"))
else:
    print(f"Load the already stored data since the flag 'calculate' is False.")
    path_, t = pickle.load(file=open(os.path.join(f"{folder_name}", "data",
                                                  f"path_{n_iter}_iter.p"), "rb"))

# compute the BHV space geodesic.
bhv_geodesic = bhv.g.s_path(p=walds[0], q=walds[1], n_points=len(path_))
ws_geodesic = path_
# determine parts of wald space geodesic that are on the boundary
on_boundary = [not np.all(p.x > 0) for p in path_]
first_index, last_index = on_boundary.index(True), len(on_boundary) - on_boundary[
                                                                      ::-1].index(True)
...
fig.pass_curve_family(curves=ws_geodesic_int, lw=2,
                      label="wald space geodesic interior", color="red")
fig.pass_curve(curve=ws_geodesic_bnd, lw=2, label="wald space geodesic boundary",
               color="brown")
...
fig.savefig(os.path.join(folder_name, f"geodesic_3d_{n_iter}_iter.pdf"))
# fig.show()
fig.close()
...
fig.savefig(os.path.join(folder_name, f"geodesic_3d_emb_{n_iter}_iter.pdf"))
# fig.show()
fig.close()
